...
const galleryImages = ['/images/pet01.jpg', '/images/pet02.jpg', '/images/pet03.jpg', '/images/pet04.jpg', '/images/pet05.jpg', '/images/pet06.jpg'];

function DashboardPage() {
  const [stats, setStats] = useState({ totalPets: 0, totalTutores: 0, totalVeterinarios: 0, totalConsultas: 0 });
  ...
  return (
    <main className="main">
      <Topbar
        ...
        action={<Link className="btn btn-primary" to="/consultas/nova">+ Nova consulta</Link>}
      />
      ...
      <section className="card">
        ...
        {erro ? <p className="filter-feedback mb-16" style={{ padding: '14px 28px 0' }}>{erro}</p> : null}
        <table>
          <thead>
            <tr>
              <th>Data</th>
      ...
      <section className="card">
        ...
        <div className="gallery-wrapper">
          <div className="gallery">
            {galleryImages.map((src) => (
              <button key={src} type="button" className="gallery-item" onClick={() => setLightbox(src)}>
                <img src={src} alt="pet" />
              </button>
            ))}
          </div>
        ...
        <div className="quick-access">
          <Link className="btn btn-filter" to="/pets/novo">Novo pet</Link>
          <Link className="btn btn-filter" to="/consultas/nova">Nova consulta</Link>
          <Link className="btn btn-filter" to="/perfil">Editar perfil</Link>
        </div>
      </section>

      <div className={`lightbox ${lightbox ? 'open' : ''}`} onClick={() => setLightbox('')}>
        <button type="button" className="lightbox-close" onClick={() => setLightbox('')}>×</button>
